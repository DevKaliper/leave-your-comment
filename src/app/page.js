      ...
      <section className="overpx-2 mt-5 grid h-full w-full grid-cols-1 items-center justify-center gap-3  md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-6">
        {comentarios.length == 0 ? (
          ...
          comentarios?.map((comentario) => (
            <div
              key={comentario.id}
              className=" card mx-auto w-64 bg-green-950 bg-opacity-70 shadow-xl ">
              <div className="card-body">
                <h2 className="card-title">{comentario.titulo}</h2>
                ...
                <div className="card-actions justify-end">
                  <div className="flex w-fit gap-1 rounded-lg border-b-2 border-green-500 px-1 py-1 duration-200 hover:bg-green-500 ">
                    {comentario.nombre ? <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
